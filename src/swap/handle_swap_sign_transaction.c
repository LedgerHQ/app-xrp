...
bool copy_transaction_parameters(create_transaction_parameters_t* params) {
    ...
    strncpy(stack_data.destination_tag, params->destination_address_extra_id, sizeof(stack_data.destination_tag));
    if ((stack_data.address[sizeof(stack_data.address) - 1] != '\0') ||
        (stack_data.destination_tag[sizeof(stack_data.destination_tag) - 1] != '\0') ||
        (params->amount_length > 8) ||
        (params->fee_amount_length > 8)) {
        return false;
    }
    ...
    memcpy(stack_data.fees + 8 - params->fee_amount_length, params->fee_amount, params->fee_amount_length);
